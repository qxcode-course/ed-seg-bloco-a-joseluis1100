...
func (m *Matrix) SaveSquareGrid(filename string) {
    ...
    for i := range m.NL {
        for j := range m.NC {
            ...
            cx := float64(j)*a + a/2
            cy := float64(i)*a + a/2
            // Adiciona o retângulo SVG para o quadrado com preencimento branco
            color := getColor(m.Get(Pos{i, j}))
            if color == "" {
                color = "white"
            }
            fmt.Fprintf(file, `<rect x="%f" y="%f" width="%f" stroke="black" fill="%s"/>` +"\n", cx, cy, svgWidth, color)
        }
    }
    // Fecha o SVG
...
func (m *Matrix) SaveHexGrid(filename string) {
    ...
    for i := range nl {
        for j := range nc {
            ...
            // Em linhas impares (começando em 0), deslocamos o centro para a direita em (sqrt(3)/2 * a)
            cx := float64(j) * math.Sqrt(3) * a
            if i%2 == 1 {
                cx += math.Sqrt(3)/2*a
            }
            //ajuste vertical: desloca o grid para baixo em "a" para evirar corte superior
            cy := float64(i)*1.5*a + a
            // Calcula as 6 vertices do hexágono
            points := ""
            for k := range 6 {
